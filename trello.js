...
function handleDrop(e) {
  ...
  if (
    dragSrcEl !== this &&
    dragSrcEl.classList.contains("card") &&
    ...
    if (draggableList.order > droppebleList.order) {
      ...
      for (let i = 0; i < data.length; i++) {
        if (
          data[i].order > droppebleListInitialOrder &&
          data[i].order < draggableListInitialOrder
        ) {
          data[i].order++;
        }
      }
    ...
    dragSrcEl !== this &&
    this.classList.contains("card-item") &&
    dragSrcEl.classList.contains("card-item")
  ) {
    const {
      task: draggableTask,
      list: draggableList,
      taskIndex: draggableTaskIndex,
    } = getTaskAndListByTaskId(dragSrcEl.id);
    const { task: droppableTask, list: droppableList } = getTaskAndListByTaskId(
      this.id
    );
    ...
    if (draggableList.id === droppableList.id) {
      ...
      if (draggableTask.order > droppableTask.order) {
        ...
        for (let i = 0; i < draggableList.items.length; i++) {
          if (
            draggableList.items[i].order > droppebleTaskInitialOrder &&
            draggableList.items[i].order < draggableTaskInitialOrder
          ) {
...
function addingListners(data) {
  for (let i = 0; i < data.length; i++) {
    addDragAndDropListnersToLists(data[i]);
  }
}
